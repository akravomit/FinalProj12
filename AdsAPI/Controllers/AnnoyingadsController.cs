...
namespace AdsAPI.Controllers
{
    ...
    public class AdvertismentDB : BaseDB<Advertisement>
    {
        protected override async Task<Advertisement> CreateModelAsync(object[] row)
        {
            ...
            advertisement.id = Convert.ToInt32(row[0]);
            advertisement.headline = row[1].ToString();
            advertisement.body = row[2].ToString();
    ...
    public abstract class DB
    {
        ...
        private const string MySqlConnSTR = $"server=localhost; user id=root; password=josh17rog;persistsecurityinfo=True;database=gameadapi";
